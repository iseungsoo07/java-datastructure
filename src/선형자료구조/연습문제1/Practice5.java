...
public class Practice5 {
    public static Integer solution(int n, int k, int l, ArrayList<ArrayList<Integer>> apples, Queue<ArrayList> moves) {
        int[][] board = new int[n + 1][n + 1];
        for (ArrayList<Integer> item : apples) {
            board[item.get(0)][item.get(1)] = 1;
        }

        Queue<ArrayList<Integer>> snake = new LinkedList<>();
        snake.add(new ArrayList<Integer>(Arrays.asList(1, 1)));

        ArrayList<ArrayList<Integer>> direction = new ArrayList<>();
        direction.add(new ArrayList<Integer>(Arrays.asList(0, 1)));
        direction.add(new ArrayList<Integer>(Arrays.asList(1, 0)));
        direction.add(new ArrayList<Integer>(Arrays.asList(0, -1)));
        direction.add(new ArrayList<Integer>(Arrays.asList(-1, 0)));
        ...
        while (true) {
            score += 1;
            x += direction.get(dIdx).get(0);
            y += direction.get(dIdx).get(1);

            if (1 <= x && x <= n && 1 <= y && y <= n) {
                ArrayList<Integer> cur = new ArrayList<Integer>(Arrays.asList(x, y));
