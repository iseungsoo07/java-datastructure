package 비선형자료구조.bst.redblack트리;
...
class RedBlackTree {
    ...
    public void insert(int key) {
        Node checkNode = null;
        if (head == null) {
            head = new Node(key, BLACK, null, null, null);
        } else {
            Node cur = head;
            while (true) {
                Node pre = cur;
    ...
    public void leftRotate(Node node) {
        if (node.parent == null) {
            Node rNode = head.right;
            head.right = rNode.left;
            rNode.left.parent = head;
            head.parent = rNode;
            rNode.left = head;
            rNode.parent = null;
            head = rNode;
    ...
    public void rightRotate(Node node) {
        if (node.parent == null) {
            Node lNode = head.left;
            head.left = lNode.right;
            lNode.right.parent = head;
            head.parent = lNode;
            lNode.right = head;
            lNode.parent = null;
            head = lNode;
